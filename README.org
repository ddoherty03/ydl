...
user to supply the data in a hierarchical series of "data definition files"
with the extension ~.ydl~. After calling ~Ydl.load~, you can access all the
data described by the ~.ydl~ files from a hierarchical tree of nested hashes
via the hash Ydl.data[<key] (or, more concisely, ~Ydl[<key]~).
...
config are automatically instantiated into Ruby objects and made available in
the ~ydl~ data tree.
